...
def create_distribution(y_train):
	distribution = []
	types = set(y_train)
	for i in types:
...
def gain(x_train, y_train, feature_index):
	entropy_gain = entropy(create_distribution(y_train))
	for data_subset in split_data(x_train, y_train, feature_index):
		entropy_gain -= entropy(create_distribution([label for (point, label) in data_subset]))
	return entropy_gain
...
def build_decision_tree(x_train, y_train, root, remaining_features, ratio=(2.0/3)):
	...
	best_feature = max(remaining_features[indices], key=lambda index: gain(x_train, y_train, index))
